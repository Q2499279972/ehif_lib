 ...
 *
 * The structure type definitions documented here are in natural (big-endian) order.
 *
 * @{
 */
#ifndef EHIF_DEFS_H_
...
typedef struct {
    ...
    uint32_t timeout;                      ///< Timeout for synchronization with transmitter at startup in units of 10 ms
...
typedef struct {
    ...
    uint16_t pinVal                  : 15; ///< Pin mask defining IO pin values (bit 0 = GIO1, bit 14 = GIO15)
