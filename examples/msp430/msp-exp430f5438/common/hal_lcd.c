...
void halLcdPrintLoopUnrolledCharSetAddress(unsigned char Line, int* pActualAddress) { 
  halLcdSetAddress(( Line * FONT_HEIGHT) << 5 ) ;           // 0x20 = 2^5
  (*pActualAddress) = LcdAddress;
}
void halLcdPrintLoopUnrolled( char String[], int j) 
{
  int i, BlockValue;
  int Address, LCD_MEM_Add, ActualAddress;
  int temp;
...
/*void halLcdPrintInnerLoop( int ActualAddress, int j, char LookUpChar) 
{
  int j, BlockValue;
  int Address, LCD_MEM_Add;
  int temp;
...
void halLcdPrintLineLoopUnrolled(char String[], unsigned char Line, int j)
{
  int temp; 
  temp = Line * FONT_HEIGHT ;
  halLcdSetAddress( temp << 5 ) ;           // 0x20 = 2^5  
  
  halLcdPrintLoopUnrolled(String, j);
}
void halLcdPrintLineLoopUnrolledChar(char theChar, int j, int *pActualAddress, char setAdd)
{
  int BlockValue;
  int Address;
  char LookUpChar;
  ...
  if (setAdd==0) {
    LookUpChar = fonts_lookup[theChar]; 
    Address = (*pActualAddress) + j*0x20;
    halLcdSetAddress(Address);
  } else {
    LookUpChar = fonts_lookup[theChar]; 
    ...
    Draw_Block_Value_Macro[4] = BlockValue >> 8;
    Draw_Block_Value_Macro[5] = BlockValue & 0xFF;
    LCD_MEM[ LcdTableAddress ] = BlockValue;
    
    halLcdSendCommand(Draw_Block_Value_Macro);
    
    LcdAddress++;
    // Multiplied by (1+16) and added by the offset
    LcdTableAddress = (LcdAddress >> 5)*17 + (LcdAddress & 0x1F); 
    (*pActualAddress)++;
  }
}
